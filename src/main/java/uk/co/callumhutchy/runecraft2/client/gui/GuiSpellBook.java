...
public class GuiSpellBook extends GuiScreen {
	...
	private static final ResourceLocation resourceLocation = new ResourceLocation(
			"runecraft2:textures/gui/spellbook.png");
	...
	public boolean airstrikeselected, hometeleportselected, waterstrikeselected, earthstrikeselected,
			firestrikeselected, earthpillarselected = false;
	...
	int epminx, epmaxx, epminy, epmaxy;
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		int iconyStart = yStart + 22;
		int iconxStart = xStart + 17;
		this.drawCenteredString(this.fontRendererObj, "Spellbook:", this.width / 2,
				(this.height / 2) - (ySize / 2) - 20, 16777215);
	...
	public void drawSkills() {
		...
		if (currentMagic >= 5) {
			if (!earthstrikeselected || currentSpell != "earthstrike") {
				mc.getTextureManager().bindTexture(this.widgetTextures);
		...
		if (currentMagic >= 8) {
			if (!firestrikeselected || currentSpell != "firestrike") {
				mc.getTextureManager().bindTexture(this.widgetTextures);
		...
		if (currentMagic >= 8) {
			if (!earthpillarselected || currentSpell != "earthpillar") {
				mc.getTextureManager().bindTexture(this.widgetTextures);
				this.drawTexturedModalRect(iconxStart + SPELL_ICON_SPACING * 5, iconyStart, 80, 0, 16, 16);
				GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
				epminx = iconxStart + (SPELL_ICON_SPACING * 5);
				epmaxx = iconxStart + (SPELL_ICON_SPACING * 5) + SPELL_ICON_SIZE;
				epminy = iconyStart;
				epmaxy = iconyStart + 16;
			}
			if (earthpillarselected || currentSpell == "earthpillar") {
				mc.getTextureManager().bindTexture(this.widgetTextures);
				this.drawTexturedModalRect(iconxStart + SPELL_ICON_SPACING * 5, iconyStart, 80, 16, 16, 16);
				GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
				setSpellsToFalse();
				earthpillarselected = true;
				epminx = iconxStart + (SPELL_ICON_SPACING * 5);
				epmaxx = iconxStart + (SPELL_ICON_SPACING * 5) + SPELL_ICON_SIZE;
				epminy = iconyStart;
				epmaxy = iconyStart + 16;
	...
	protected void mouseClicked(int varx, int vary, int p_73864_3_) {
		...
		if (epminx < varx && varx < epmaxx && epminy < vary && vary < epmaxy) {
			earthpillarselected = !earthpillarselected;
		}
		if (earthpillarselected) {
			props.currentSpell = "earthpillar";
			setSpellsToFalse();
			earthpillarselected = true;
		}
	}

	private void setSpellsToFalse() {
	...
	protected boolean IsButtonMouseovered(int varx, int vary, GuiButton button) {
		...
		if (htminx < varx && varx < htmaxx && htminy < vary && vary < htmaxy) {
			...
			this.DrawTooltipScreen(varx, vary, "_lHome Teleport:_r _p" + "No rune cost. _p"
					+ "Set your Home location with _o/sethometeleport_r _p" + "This will teleport you home.");
		}
		if (wsminx < varx && varx < wsmaxx && wsminy < vary && vary < wsmaxy) {
			this.DrawTooltipScreen(varx, vary,
					"_lWater Strike:_r _p" + "1x Water Rune _p" + "Fires a weak bolt of water.");
		}
		if (fsminx < varx && varx < fsmaxx && fsminy < vary && vary < fsmaxy) {
			this.DrawTooltipScreen(varx, vary,
					"_lFire Strike:_r _p" + "1x Fire Rune _p" + "Fires a weak bolt of fire.");
		}
		if (esminx < varx && varx < esmaxx && esminy < vary && vary < esmaxy) {
			this.DrawTooltipScreen(varx, vary,
					"_lEarth Strike:_r _p" + "1x Earth Rune _p" + "Fires a weak bolt of earth.");
		}
		if (epminx < varx && varx < epmaxx && epminy < vary && vary < epmaxy) {
			this.DrawTooltipScreen(varx, vary,
					"_lEarth Pillar:_r _p" + "2x Earth Rune _p" + "Fires a weak bolt of water.");
		}
