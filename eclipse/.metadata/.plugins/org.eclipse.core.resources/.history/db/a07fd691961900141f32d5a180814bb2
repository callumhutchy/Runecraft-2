				...
				case "airrunealtar":
					try {
						...
						if (props.isSkillHighEnough("runecrafting", 1)) {
							boolean test = false;
							TileEntityAirRuneAltar tileEntity = (TileEntityAirRuneAltar) world.getTileEntity(varx, vary, varz);
							if (item == Items.staff || item == Items.staffofair || item == Items.staffofearth || item == Items.staffoffire || item == Items.staffofwater && world.getBlock(varx, vary, varz) == Blocks.AirRuneAltar) {
